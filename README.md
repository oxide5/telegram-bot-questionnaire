...
A lightweight and efficient Telegram bot built with **Python** and **aiogram 3.x**. The bot serves as a bridge between users and an administrator, forwarding messages to a specific ID and logging all interactions into a sqlite file for persistent storage.

## Features

* **Message Forwarding:** Automatically forwards any message sent by a user to a predefined Admin ID.
* **Data Logging:** Every interaction is saved locally in a `users.db` file, ensuring no information is lost.
